# Spec — binding `lyraAppSidebar` para o `@lyra-ds/alpine`

> Para: a sessão que conduz o monorepo `lyra` (`packages/alpine`).
> De: a sessão do `lyra-ds/blade`, 2026-08-08, durante a onda B das ondas B–F.
...
A spec das ondas B–F classificou `app-sidebar` como um dos **"3 estáticos (zero
...
upstream.** A task 7 do plano `.batuta/plan-alpine-0-2-ondas-b-f.md` fica bloqueada
...
- **`chevron`** *(a decidir, ver §4)* — alguma forma de trocar o `d` do path.
