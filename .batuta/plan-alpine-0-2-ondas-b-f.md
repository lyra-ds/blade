...
- [x] 1. Matriz de compatibilidade do README: `@lyra-ds/alpine` `^0.1.1` → `^0.2` — maestro (claude)
      Accept: `README.md:83` atualizado; `npm view @lyra-ds/alpine version` = 0.2.0 confirmado. **FEITO nesta sessão (pendente de commit junto da task 2).**
- [ ] 2. Bump do `blade-demo` para `@lyra-ds/alpine` `^0.2.0` + `npm install` + `npm run build` — codex (medium)
      Accept: `blade-demo/package.json` em `^0.2.0`, lockfile atualizado, `vite build` verde, galeria atual (50 componentes) sem regressão visual óbvia; commit no repo do demo.
- [ ] 3. Componente `code-block` (binding `lyraCodeBlock`, S) — codex (medium)
      Accept: markup servido com `x-bind` `copyButton`/`status`, `data-copy-text` opcional, live-region `role="status"`; class-emission contra o fixture do React; suíte verde; guidelines regeneradas idempotentes; pint limpo.
